 ...
 *  坐标系转换
 *
 *  copy from https://github.com/taoweiji/JZLocationConverter-for-Android/blob/master/JZLocationConverter.java
 *
 * @author zhangjw
 * @date 2022/11/2 0002 10:52
 */
public final class CoordinateConverter {
    ...
    private static Location gcj02Encrypt(double ggLat, double ggLon) {
        Location resPoint = new Location();
        double mgLat;
        double mgLon;
    ...
    private static Location gcj02Decrypt(double gjLat, double gjLon) {
        Location gPt = gcj02Encrypt(gjLat, gjLon);
    ...
    private static Location bd09Decrypt(double bdLat, double bdLon) {
        Location gcjPt = new Location();
    ...
    private static Location bd09Encrypt(double ggLat, double ggLon) {
        Location bdPt = new Location();
     ...
     */
    public static Location gcj02ToWgs84(Location location) {
     ...
     */
    public static Location wgs84ToBd09(Location location) {
        Location gcj02Pt = gcj02Encrypt(location.latitude, location.longitude);
     ...
     * ####此接口有1－2米左右的误差，需要精确定位情景慎用
     */
    public static Location bd09ToWgs84(Location location) {
        Location gcj02 = bd09ToGcj02(location);
        return gcj02Decrypt(gcj02.latitude, gcj02.longitude);
    }

    public static void main(String[] args) {
        Location from = new Location(30.72346497, 121.34269714);
        Location to = wgs84ToGcj02(from);
        System.out.println(to.latitude + "," + to.longitude);
        to = wgs84ToBd09(from);
        System.out.println(to.latitude + "," + to.longitude);
    }
